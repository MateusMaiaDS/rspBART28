...
nodeLogLike <- function(curr_part_res,
  ...
  if(length(j_)==0){
    ...
    # In case where there are predictors
...
remove_variable <- function(tree,
  ...
  # Pruning a node
  #==== THIS GONNA BE REMOVED now it's gonna be possible to remove the main effect =====#
  valid_terminal_node <- TRUE
  valid_count <- 0
  ...
  if(length(p_node$pred_vars)==0){
    return(tree)
  }

  # Attention on this line!!
  if(length(p_node$pred_vars)==1){
    remove_var <- p_node$pred_vars
  } else {
...
updateBetas <- function(tree,
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    # Case where there are no betas
    if(length(node_index_var)!=0){
    ...
    } else {   # Else if for the verifycation if there's any beta
      # Do nothing!
    }
  }
...
update_tau <- function(y_train_hat,
  ...
  for(tree in 1:length(forest)){
    ...
    for(curr_node in terminal_nodes_names) {
      ...
      # Getting the length of the predicted vars
